...
var sum = function(array) {
  if (array.length > 1) {
    ...
    var newOne = array.slice(0, array.length-1);
    var lastOne = copy.pop()
    return lastOne + sum(newOne);
    ...
    if (Array.isArray(array)) {
        for (let x = 0; x < array.length; x++) {
            ...
            if (Array.isArray(inside)) {
                var insideNum = arraySum(inside);
                total += insideNum;
            } else {
                total += parseInt(inside);
            }
...
var exponent = function(base, exp) {
    ...
    //for negative exp
    if (exp < 0) {
      //returns before reaching the one case
      if (exp + 1 === 0) {
        var rounded = 1 / base;
        return rounded;
      } 
      return 1/base * Number.parseFloat(exponent(base, exp + 1)).toPrecision(4);
    }
